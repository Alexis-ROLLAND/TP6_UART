...
uart_err_t      uart_putch(uart_id_t uart_id, uint8_t Car, uart_tx_blocking_t BlockingMode)
{
    ...
    {
        case _UART1:
            if (U1STAbits.UTXBF == 0)   // Si le buffer Tx est libre, on ?met sans attendre, quelquesoit le mode (bloquant ou non)  
            {
                U1TXREG = Car;
                return UART_OK;
            }
            else
            {
                ...
                while (U1STAbits.UTXBF == 1);   // Attente lib?ration buffer
                U1TXREG = Car;
            }
        
        case _UART2:
            if (U2STAbits.UTXBF == 0)   // Si le buffer Tx est libre, on ?met sans attendre, quelquesoit le mode (bloquant ou non)  
            {
                U2TXREG = Car;
                return UART_OK;
            }
            ...
            {
                if (BlockingMode == UART_TX_NON_BLOCKING_MODE) return UART_TX_FIFO_FULL;    // Mode non bloquant, buffer plein, on retourne une erreur
                while (U2STAbits.UTXBF == 1);   // Attente lib?ration buffer
                U2TXREG = Car;
            }
        default: 
            return UART_UNKNOWN_UART;
